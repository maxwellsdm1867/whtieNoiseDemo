...
class SyntheticDataGenerator:
    ...
    def generate_white_noise_stimulus(self, n_time_bins: int, 
        ...
        stimulus = np.random.normal(0, contrast_std, stimulus_shape)
    ...
    def generate_responses(self, stimulus: np.ndarray, bin_size: float = 0.008) -> np.ndarray:
        ...
        if self.noise_level > 0:
            ...
            noise_factor = 1.0 + self.noise_level * np.random.randn(len(expected_spike_count))
        ...
        spike_counts = np.random.poisson(expected_spike_count)
