...
class SingleCellAnalyzer:
    ...
    def _estimate_nonlinearity_from_chunks(self, stimulus_chunks: List[np.ndarray], 
        ...
        if method == 'nonparametric':
            ...
            self.nonlinearity_estimator = NonparametricNonlinearity(n_bins=n_bins)
            
            # Fit the nonlinearity
            self.nonlinearity_estimator.fit(full_generator_signal, full_spike_counts)
            
            # Extract nonlinearity data from fitted estimator
            if self.nonlinearity_estimator.bin_centers is not None:
                x_values = self.nonlinearity_estimator.bin_centers
                y_values = self.nonlinearity_estimator.spike_rates
            ...
            self.nonlinearity_estimator = ParametricNonlinearity(model_type=model_type)
            
            # Fit the nonlinearity
            self.nonlinearity_estimator.fit(full_generator_signal, full_spike_counts)
            
            # Get parameters and generate curve
            params = self.nonlinearity_estimator.parameters
            x_range = np.linspace(
                np.min(full_generator_signal), 
                np.max(full_generator_signal), 
                100
            )
            y_pred = self.nonlinearity_estimator.predict(x_range)
